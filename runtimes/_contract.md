...
A runtime module teaches `ssmd` how to build, serve, and operate one language
stack. Adding PHP 7.2, Bun, Rust or Elixir means adding a directory here - never
editing `ssmd`, `lib/`, or `docker-compose.yml`.
...
runtimes/<kind>/
  ...
  entrypoint.sh     # installed as /usr/local/bin/ssmd-entrypoint
  commands.sh       # sourced by ssmd; defines the rt_* functions below
...
process needs to write must live under `/ssmd/cache` (a bind mount, created by ssmd)
or be world-writable in the image.

## entrypoint.sh

Installed as `ssmd-entrypoint`. Invoked as `ssmd-entrypoint <role>`, where role is:

| Role | Contract |
|---|---|
| `serve` (default) | Run the web server in the foreground on `$SSMD_PORT`. Must answer `$SSMD_HEALTHZ` with 200 **from the web server itself**, not the application framework. |
...
`$SSMD_HEALTHZ` answered by the web server is non-negotiable. A healthcheck that
...
`SSMD_STACK`, `SSMD_DOMAIN`, `SSMD_RUNTIME`, `SSMD_FRAMEWORK`, `SSMD_DOCROOT`, `SSMD_PORT`,
`SSMD_HEALTHZ`, `SSMD_INSTANCE`, `SSMD_ROLE`, plus `DB_*`, `REDIS_*`, `MAIL_*`, `S3_*`.

`SSMD_INSTANCE` is `main` for the base stack and the slug for a worktree or agent
sandbox. Use it for anything that must not collide between instances - a cache
key prefix, a lock name, a log filename.

## commands.sh

Sourced into ssmd's shell, so it may use `log`, `die`, `dexec`, `compose`,
`container`, and every `STACK_*` variable. Define these:

| Function | Contract |
|---|---|
| `rt_display_name` | One line, human-readable. Shown by `ssmd describe`. |
| `rt_deps_install` | Install dependencies inside the app container. Called by the `postCreate` hook and `ssmd deps`. |
...
| `rt_dispatch <cmd> [args...]` | Handle runtime-specific verbs. Return 1 for anything unrecognised so ssmd can print its own error. |
| `rt_verbs` | Space-separated list of verbs `rt_dispatch` handles. Feeds `ssmd help` and shell completion. |
...
hooks, `ssmd run`, `ssmd exec`, and the per-instance equivalents. If a runtime needs
...
`ssmd test` never calls the framework's test runner directly, and
