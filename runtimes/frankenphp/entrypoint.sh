...
: "${DX_FRAMEWORK:=none}"
: "${DX_INSTANCE:=main}"
export DX_PORT DX_DOCROOT DX_HEALTHZ
...
# Trust the stack's local CA so that server-side HTTP to https://*.$DX_DOMAIN
...
if [ -f /dx/ca/root.crt ] && [ ! -f /usr/local/share/ca-certificates/dx-local.crt ]; then
    if cp /dx/ca/root.crt /usr/local/share/ca-certificates/dx-local.crt 2>/dev/null; then
        ...
        say "stack CA available at /dx/ca/root.crt (not root; using CURL_CA_BUNDLE)"
    fi
fi
...
case "$ROLE" in
    serve)
        say "frankenphp $(php -r 'echo PHP_VERSION;') serving /app/${DX_DOCROOT} on :${DX_PORT} (instance ${DX_INSTANCE})"
        exec frankenphp run --config /etc/caddy/Caddyfile
        ;;

    queue)
        cmd="$(framework_queue_cmd)"
        [ -n "$cmd" ] || { say "framework '$DX_FRAMEWORK' has no queue worker - set services.queue: false"; exit 1; }
        wait_for_deps
        say "$cmd"
    ...
    scheduler)
        ...
        [ -n "$cmd" ] || { say "framework '$DX_FRAMEWORK' has no scheduler - set services.scheduler: false"; exit 1; }
        wait_for_deps
    ...
    *)
        # Anything else is run verbatim, which is what `dx exec` relies on.
        exec "$@"
        ;;
esac
