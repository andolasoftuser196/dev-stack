# CLAUDE.md - working on the dx toolkit itself

Guidance for Claude Code when the task is changing **this repository**. For
working *inside* a dx stack on some other project, the plugin's
`dev-stack-ops` skill is the right reference; this file is about the toolkit.

## What this is

A generic development environment: one `dx` bash script over a profile-gated
...
in `dx`, `lib/`, a compose file or a Dockerfile, it belongs in
...
**`dx` runs on bash, docker, git and sqlite3.** SQLite is the one dependency the
...
subsequent `dx` reads a flat file. **That cache is not an optimisation to remove
later**; it is what keeps dx fast and what keeps it usable when things are broken.

`dx init` additionally needs python with jinja2, and it happens once.
...
**Language-specific behaviour goes in `runtimes/<kind>/`, never in `dx` or
...
`dx`, the seam is gone - and it went first in every stack this replaces.

There are two deliberate exceptions, both in `lib/core.sh`, both defaulting a
value rather than branching behaviour (`DX_APP_PORT`, the image tag). If you add
...
project; change `config/stack.yml`, or `dx config set`.
...
| `dx wt` | `lib/worktree.sh` |
| `dx agent` | `lib/agent.sh` |
...
**`dx test` refuses a non-disposable database name.** Both the runtime modules
...
**The review gate holds; it does not block.** `dx agent diff` returns a verdict
...
missing there becomes a container `dx down` silently leaves running.
...
`dx run`, `dx exec` and the per-instance equivalents all go through it.

**No non-interactive path uses a login shell.** `/etc/profile` resets `PATH` on
Debian and drops the language toolchain. `dx sh` is the one exception, because
...
**Row-returning SQL is separated by `DX_FS` (U+001F), never a tab.** Tab is IFS
...
./dx describe                                # does it load?
./dx preflight                               # does the host check work?
