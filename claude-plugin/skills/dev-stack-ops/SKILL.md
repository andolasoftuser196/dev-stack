---
name: dev-stack-ops
description: Operate a ssmd development environment - start and stop it, run commands in it, read its logs, inspect its data, and check whether the app actually works. Use whenever a task involves running this project locally, and BEFORE reaching for raw docker, docker compose, or a framework CLI on the host.
---

# Working in a ssmd dev-stack

This project's local environment is driven by `ssmd`, a single script in the
...
```bash
./ssmd describe    # runtime, framework, services, the verbs this project has
./ssmd status      # what is running, and where it is
```

`ssmd describe` tells you whether to use `ssmd artisan`, `ssmd cake`, `ssmd manage` or
`ssmd go` - do not guess, and do not assume from file extensions.

## Tool order

Reach for these in order. Each one is faster and more reliable than the next.

1. **`./ssmd run <cmd>`** - anything the framework can answer. A REPL one-liner, a
   CLI command, `migrate:status`. This is almost always the right tool.
2. **`./ssmd db:query "<sql>"`** - anything about *data* rather than code. Faster
   than an ORM round-trip and the output is easier to read.
3. **`./ssmd logs <service> -f`** - before guessing at a cause. Filter it; an
   unfiltered log is mostly request noise.
4. **`./ssmd verify`** - after making a change. See below; this is the strongest
   signal available.
...
6. **A browser**, last. It is the slowest and most fragile tool here. Use
   `./ssmd browse <url>` and watch it at `https://vnc.<domain>/`, and only when the
   task genuinely requires driving the UI.

## `ssmd verify` is the check that matters

```bash
./ssmd verify              # the main stack
./ssmd verify <slug>       # a worktree or agent instance
```
...
A passing `ssmd verify` is evidence. "The tests pass" is weaker - the suite may not
...
|---|---|---|
| `ssmd preflight` | Will `ssmd up` work on this machine? | `ssmd up` failed for a non-obvious reason |
| `ssmd doctor` | Does reality match what ssmd believes? | An instance is behaving oddly; after manual docker fiddling |
| `ssmd verify` | Is the app working right now? | After every change |

None of them modify anything. `ssmd doctor` in particular never auto-fixes - it
...
- **`stack.yml`** - what this project *is*: runtime, framework, services, routes,
  lifecycle hooks. Committed. Edit it and any `ssmd` command picks the change up
...
**Process environment beats the app's `.env` file.** ssmd injects `DB_HOST`,
...
connection does nothing. Edit `stack.yml` (or `dev-stack/.env`) and `ssmd restart`.

**Code changes need no restart; some things do.**
- Interpreted runtimes revalidate on every request - just save.
- The **queue worker** caches the booted framework: `ssmd recreate queue`.
- A **Caddyfile** edit: the proxy reloads on `ssmd wt`/`ssmd agent` operations, or
  `ssmd exec proxy caddy reload --config /etc/caddy/Caddyfile`.
- `php.prod.ini` locks opcache: `ssmd recreate app` after every edit.

**Never run the test suite outside `ssmd test`.** A suite that refreshes the schema
reads whatever database the config names and drops every table in it. `ssmd test`
...
lockfile, and that diff needs a human. `ssmd deps` does the frozen-lockfile install.
...
trust it. If *your* browser or a host-side tool does not, run `./ssmd ca-cert` - it
