...
allowed-tools: Bash(*/ssmd wt*), Bash(*/ssmd verify*)
---

Run `./ssmd wt $ARGUMENTS`.
...
- Redis has fifteen usable logical databases, so fifteen concurrent instances is
  a hard ceiling. `ssmd wt ls` shows what is allocated.
- `ssmd wt rm` deletes a git worktree. Check for uncommitted work first
