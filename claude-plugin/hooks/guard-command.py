...
call in every session where the plugin is enabled, so:
  ...
  - it does nothing at all outside a ssmd project (find_ssmd_root returns None);
...
from ssmd_common import (add_context, audit, compile_rule, deny,  # noqa: E402
                       find_ssmd_root, load_rules, passthrough, read_input)
...
def main() -> None:
    ...
    # A command that already goes through ssmd is by definition allowed: ssmd's own
    ...
    #
    # Anchored to a path ending in /ssmd or a bare `ssmd`/`./ssmd` at the start of a
    # pipeline segment, so that a command merely mentioning "ssmd" in an argument
    # does not slip through.
    if re.search(r"(^|[;&|]\s*)(\./|/\S*/)?ssmd(\s|$)", cmd):
        audit(ssmd_root, "bash", cmd, "allow:ssmd")
        passthrough()

    for pattern, reason in load_rules(ssmd_root / "policy" / "denied-commands.tsv"):
        rx = compile_rule(pattern)
        if rx is None:
            # A malformed rule is a bug in the policy file, not grounds to block
            # the user's work. Skip it; `ssmd agent policy` is where it gets noticed.
            continue
        if rx.search(cmd):
            audit(ssmd_root, "bash", cmd, "deny")
    ...
    audit(ssmd_root, "bash", cmd, "allow")

    # One nudge, not a block: a framework CLI run directly still works, it just
    # skips the environment ssmd would have set up. Saying so once is more useful
    ...
    if re.search(r"(^|[;&|]\s*)(php artisan|bin/cake|python manage\.py|npm run|go run)\s", cmd):
        add_context(EVENT,
                    ...
                    "equivalent is `./ssmd run <command>`.")
