# Contributing to ssmd

Thanks for looking. ssmd is a small toolkit with strong opinions, and most of
...
cd dev-stack
./ssmd describe          # does the config layer load?
./ssmd preflight         # does this machine satisfy the host checks?
tests/run              # unit + python - no docker daemon needed
```

Daily use needs docker with the compose v2 plugin, bash, git and sqlite3.
`ssmd init` additionally needs python3 with jinja2, and only `ssmd init`.
...
`ssmd`, `lib/`, a compose file or a Dockerfile, it belongs in `config/defaults.yml`
...
moment a `case "$STACK_RUNTIME_KIND"` appears in `ssmd` or `lib/`, the seam is
...
project - change `config/stack.yml`, or use `ssmd config set`.

**4. Dependencies stay at bash, docker, git and sqlite3.** The `.stack.env`
cache is what keeps ssmd fast and what keeps it usable when things are broken; it
...
- the image bakes in no UID, and writes only under `/ssmd/cache`
- `entrypoint.sh` answers `$SSMD_HEALTHZ` **from the web server**, not the
  framework, and handles the `serve`, `queue`, `scheduler` and `idle` roles
- `ssmd test` refuses a non-disposable database name
- a demo app lands in `demo-apps/<name>/` and a stack config in
  `examples/runtimes/<kind>-<name>.stack.yml`, so `ssmd up` works with no project
...
Open an issue with the output of `./ssmd doctor` and `./ssmd describe`. Both are
read-only and neither prints secrets. If it involves a container, `./ssmd logs`
