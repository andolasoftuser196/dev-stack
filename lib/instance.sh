# lib/instance.sh - the machinery both `dx wt` and `dx agent` are built on.
#
...
#     concurrent `dx wt add` calls could both read "3 is free".
...
EOF
    ...
    [ -n "$line" ] || die "no such instance '$slug' (dx wt ls)"

    IFS="$DX_FS" read -r INSTANCE_SLUG INSTANCE_KIND INSTANCE_BRANCH INSTANCE_DIR \
...
instance_register() {
    printf 'INSERT INTO instances(slug,kind,branch,worktree,database,redis_db,created,owner) VALUES(%s,%s,%s,%s,%s,%s,%s,%s);' \
        ...
        || die "could not register instance '$1' - is the slug or redis database already taken? (dx wt ls)"
...
EOF
    ...
    [ -n "$n" ] || die "all Redis logical databases ${lo}-${hi} are allocated.
      That is the concurrency ceiling. Free one: dx wt ls, then dx wt rm <slug>
      (or raise it, if your Redis has more: dx config set instances.redis_db_max N)"
    echo "$n"
}
...
# Generated by dx for instance '${slug}'. Removed on teardown.
# Do not edit: the next 'dx ${INSTANCE_KIND:-wt} up ${slug}' overwrites it.
http://${slug}.${STACK_DOMAIN}, https://${slug}.${STACK_DOMAIN} {
	reverse_proxy ${upstream} {
		flush_interval -1
		header_up X-Dx-Instance ${slug}
	}
}
EOF
...
instance_reload_proxy() {
    ...
    warn "proxy reload failed - the route is written but not live.
       Check the config:  dx exec proxy caddy validate --config /etc/caddy/Caddyfile"
    return 1
}

# ── /etc/hosts ──────────────────────────────────────────────────────────────
# /etc/hosts cannot express a wildcard, so without wildcard DNS every instance
# needs its own line. Writing it requires root, which dx will not take silently:
...
instance_hosts_rm() {
    ...
    sudo sed -i.dxbak "/[[:space:]]${host}\$/d" /etc/hosts 2>/dev/null || true
}
...
instances_list() {  # [kind]
    ...
    if [ "$(instances_count)" = "0" ]; then
        echo "  no instances. Create one:  dx wt add <branch>"
        return 0
    fi
    printf '  %-24s %-6s %-26s %-9s %-6s %-8s %s\n' SLUG KIND BRANCH STATE REDIS LEASE URL
    while IFS="$DX_FS" read -r slug kind branch dir dbname rdb created owner; do
        [ -z "${slug:-}" ] && continue
