# lib/agent.sh - `dx agent`: give a coding agent its own isolated environment.
#
...
agent_usage() {
    cat <<'EOF'
dx agent - isolated environments for coding agents

  dx agent spawn <branch> [--slug s] [--owner who] [--ttl 4h] [--egress none|allowlist|full]
  dx agent ls
  dx agent up <slug> | stop <slug>    restart or stop an existing sandbox
  dx agent attach <slug>              shell inside the sandbox (where the agent runs)
  dx agent run <slug> <cmd...>        run one command inside the sandbox
  dx agent logs <slug> [-f]
  dx agent verify <slug>              did the agent's change actually work?
  dx agent diff <slug>                what changed, plus the policy verdict
  dx agent rm <slug> [--drop-db] [--delete-branch]
  dx agent reap                       remove sandboxes whose lease has expired
  dx agent policy                     show the active policy
  dx agent audit [-n 50] [--actor who] [--event kind]
...
Isolation (see docs/AGENTS.md):
  ...
  policy     denied paths + size caps evaluated by 'dx agent diff'
EOF
}

agent_spawn() {
    local branch="" slug="" owner="${DX_ACTOR:-${USER:-agent}}" \
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            -*) die "unknown flag '$1' (dx agent spawn --help)" ;;
  ...
  From inside the sandbox: the worktree at /app, dx read-only at /dx, this
  instance's app at \$DX_APP_URL (address it by that or by '${slug}' - never by
...
agent_attach() {
    local slug="${1:?usage: dx agent attach <slug>}"
    instance_load "$slug"
    [ "$INSTANCE_KIND" = "agent" ] || die "'$slug' is a $INSTANCE_KIND instance, not an agent sandbox"
    local c="${PROJECT}-agent-${slug}-sandbox"
    docker ps --format '{{.Names}}' | grep -qx "$c" || die "sandbox container '$c' is not running (dx agent ls)"
    audit "agent.attach" "$slug"
    dexec "$c" bash -l
}

agent_run() {
    local slug="${1:?usage: dx agent run <slug> <cmd...>}"; shift
    instance_load "$slug"
...
agent_diff() {
    local slug="${1:?usage: dx agent diff <slug>}"
    instance_load "$slug"
...
# explicit command rather than something `dx up` does behind your back.
agent_reap() {
    local reaped=0 slug
    ...
    for row in "${rows[@]}"; do
        [ -z "$row" ] && continue
        IFS="$DX_FS" read -r slug kind _b _d _db _r _c owner <<< "$row"
        [ "$kind" = "agent" ] || continue
...
agent_audit() {
    ...
    while IFS="$DX_FS" read -r ts actor event detail; do
        [ -z "$ts" ] && continue
        found=1
...
agent_dispatch() {
    ...
    case "$sub" in
        ...
        # `dx wt up` deliberately never starts a sandbox, so restarting a stopped
        # agent instance needs its own verb rather than a flag on the other one.
        up|start)   [ $# -ge 1 ] || die "usage: dx agent up <slug>"
                    instance_load "$1"
                    [ "$INSTANCE_KIND" = agent ] || die "'$1' is a $INSTANCE_KIND instance - use 'dx wt up $1'"
        ...
        verify)     [ $# -ge 1 ] || die "usage: dx agent verify <slug>"; cmd_verify "$1" ;;
        diff)       agent_diff "$@" ;;
        rm|remove)  wt_rm "$@" ;;
        reap)       agent_reap ;;
        policy)     policy_show ;;
