...
#
# All configuration lives in a SQLite database. Nothing in dx, lib/, the compose
...
#
#   host:<name>   config/hosts.yml     this machine     (DX_HOST selects one)
...
# configuration is diffable, reviewable and committable; the database is what dx
# reads, and `dx config set` changes it without touching a file. Editing a seed
# re-imports it automatically (keyed on mtime), and `dx config export` writes the
# database back out so a runtime change can be committed.
#
# Why a database rather than the files directly:
#   - `dx config set` works at runtime, from a script, or over MCP, with no
...
#
# Why a flat cache on top: dx runs dozens of times an hour and must stay fast and
...
config_import_file() {  # <path> <scope> [strip_first_segment]
    ...
    local sql; sql="$(mktemp)"
    {
        ...
        done < <(awk -v mode=dotted -f "$DX_ROOT/lib/yaml.awk" "$path" 2>/dev/null)
...
SELECT $(sq_quote "$(_config_now)"), $(sq_quote "${DX_ACTOR:-${USER:-unknown}}"),
...
config_host_scopes() {
    awk -v mode=dotted -f "$DX_ROOT/lib/yaml.awk" config/hosts.yml 2>/dev/null \
        | cut -d. -f1 | sort -u
}
...
SELECT key, value FROM (
  SELECT key, value,
         ROW_NUMBER() OVER (
           PARTITION BY key
           ORDER BY CASE scope
                      WHEN $(sq_quote "host:${DX_HOST:-local}") THEN 1
                      WHEN 'stack' THEN 2
                      WHEN 'default' THEN 3
                      ELSE 9 END
         ) AS rank
    FROM config
   WHERE scope IN ($(sq_quote "host:${DX_HOST:-local}"), 'stack', 'default')
) WHERE rank = 1 ORDER BY key;
EOF
}

# Turn a dotted key into the shell variable dx and compose read.
_config_varname() {
    printf 'STACK_%s' "$(printf '%s' "$1" | tr '[:lower:].-' '[:upper:]__')"
}

config_build_cache() {
    local tmp; tmp="$(mktemp "${TMPDIR:-/tmp}/dx-cache.XXXXXX")"
    {
        echo "# Generated from the config database. Do not edit - 'dx config set' instead."
        echo "# host=${DX_HOST:-local}  generated=$(_config_now)"
        local key value
        while IFS="$DX_FS" read -r key value; do
            [ -z "$key" ] && continue
    ...
    if [ ! -s "$tmp" ]; then rm -f "$tmp"; die "config resolution produced nothing - is the database seeded? try: dx config import"; fi

    if ! mv "$tmp" "$CONFIG_CACHE" 2>/dev/null; then
        # Read-only toolkit (an agent sandbox mounts /dx ro). Keep the resolved
        # cache in the temp directory instead of failing: an agent must still be
        # able to run dx from inside its sandbox.
        CONFIG_CACHE="$tmp"
    fi
}
...
config_cache_stale() {
    ...
    [ "$DX_DB_PATH" -nt "$CONFIG_CACHE" ] && return 0
    local f
    for f in $CONFIG_SEEDS; do [ -f "$f" ] && [ "$f" -nt "$CONFIG_CACHE" ] && return 0; done
    # A different DX_HOST resolves differently, and the cache does not know that
    # by mtime alone - so it records which host it was built for.
    grep -q "^# host=${DX_HOST:-local} " "$CONFIG_CACHE" 2>/dev/null || return 0
    return 1
}
...
config_get() {  # <key>
    local key="$1"
    [ -n "$key" ] || die "usage: dx config get <key>"
    local v; v="$(config_resolve_sql | sq | awk -F"$DX_FS" -v k="$key" '$1==k {print $2; exit}')"
...
config_set() {  # <key> <value> [--scope stack|default|host:<n>]
    ...
    [ -n "$key" ] || die "usage: dx config set <key> <value> [scope]"
...
BEGIN;
INSERT INTO config_history(ts, actor, scope, key, old_value, new_value)
VALUES($(sq_quote "$(_config_now)"), $(sq_quote "${DX_ACTOR:-${USER:-unknown}}"),
...
EOF
    ...
    hint "this is now out of step with config/$( [ "$scope" = default ] && echo defaults || echo "${scope%%:*}" ).yml - 'dx config export' writes it back"
}
...
config_unset() {  # <key> [scope]
    ...
    local old; old="$(printf 'SELECT value FROM config WHERE scope=%s AND key=%s;' \
                      "$(sq_quote "$scope")" "$(sq_quote "$key")" | sq1)"
    [ -n "$old" ] || die "$scope has no '$key' to unset (dx config explain $key)"
    # Recorded, like every other change. A removal nobody can attribute is the
    # one people spend longest chasing.
    cat <<EOF | sq >/dev/null
BEGIN;
INSERT INTO config_history(ts, actor, scope, key, old_value, new_value)
VALUES($(sq_quote "$(_config_now)"), $(sq_quote "${DX_ACTOR:-unknown}"),
...
config_list() {  # [prefix]
    local prefix="${1:-}"
    printf '  %-38s %-24s %s\n' KEY VALUE FROM
    cat <<EOF | sq | while IFS="$DX_FS" read -r k v s; do
SELECT key, value, scope FROM (
  SELECT key, value, scope,
         ROW_NUMBER() OVER (PARTITION BY key ORDER BY
           CASE scope WHEN $(sq_quote "host:${DX_HOST:-local}") THEN 1
                      WHEN 'stack' THEN 2 WHEN 'default' THEN 3 ELSE 9 END) AS rank
    FROM config WHERE scope IN ($(sq_quote "host:${DX_HOST:-local}"), 'stack', 'default')
...
config_explain() {  # <key>
    local key="$1"
    [ -n "$key" ] || die "usage: dx config explain <key>"
    echo "  $key"
    echo
    printf '    %-16s %-30s %s\n' SCOPE VALUE ORIGIN
    cat <<EOF | sq | while IFS="$DX_FS" read -r s v o u; do
SELECT scope, value, origin, updated FROM config WHERE key = $(sq_quote "$key")
 ORDER BY CASE scope WHEN $(sq_quote "host:${DX_HOST:-local}") THEN 1
                     WHEN 'stack' THEN 2 WHEN 'default' THEN 3 ELSE 9 END;
EOF
        local mark=" "; [ "$s" = "host:${DX_HOST:-local}" ] && mark="*"
    ...
    echo
    echo "    effective: $(config_get "$key" 2>/dev/null || echo '(unset)')   [host=${DX_HOST:-local}]"
...
config_history() {  # [key]
    ...
    cat <<EOF | sq | while IFS="$DX_FS" read -r ts actor scope k old new; do
...
# Write the database back out to the seed files, so a runtime `dx config set`
...
config_export() {
    ...
    case "$scope" in
        ...
        *) die "usage: dx config export [stack|default|host:<name>]" ;;
    esac
    ...
    echo
    hint "dx does not rewrite the seed files: doing so would strip the comments,"
    hint "which are most of what defaults.yml is for. Copy the lines above into"
    hint "$file by hand, then 'dx config import' to mark them as imported."
}
...
config_dispatch() {
    ...
    case "$sub" in
        ...
        hosts)    echo "  available (DX_HOST in .env selects one):"
                  config_host_scopes | sed "s/^/    /;s/\$/$( [ -n "${DX_HOST:-}" ] && echo '' )/"
                  echo; echo "  active: ${DX_HOST:-local}" ;;
        path)     echo "$DX_DB_PATH" ;;
        *) cat <<'EOF'
dx config - configuration lives in SQLite, seeded from config/*.yml

  dx config list [prefix]        every effective value, and which layer it came from
  dx config get <key>            one effective value
  dx config set <key> <v> [scope]  change it now (scope: stack | default | host:<n>)
  dx config unset <key> [scope]
  dx config explain <key>        why this value - every layer, and the shell variable
  dx config history [key]        what changed, when, and who changed it
  dx config import               re-read config/*.yml into the database
  dx config export [scope]       runtime changes not yet written back to the seeds
  dx config hosts                host profiles available; DX_HOST selects one
  dx config path                 where the database is

Layers, highest first:  host:<DX_HOST>  >  stack  >  default
EOF
           ;;
    esac
}
